defmodule Day17_two do
  ...
  # create a map with key a tuple coordinate {x,y,z,w}
  def format(file) do
    file
    |> Enum.with_index()
    |> Enum.map(fn {string, y_index} ->
      ...
      |> String.graphemes()
      |> Enum.with_index()
      |> Enum.map(fn {state, x_index} -> {{x_index, y_index, 0, 0}, state} end)
    end)
    |> List.flatten()
    |> Map.new()
  end

  def all_neighbours({x, y, z, w}) do
    goes_through_all_x({x, y, z, w}) -- [{x, y, z, w}]
  end

  def goes_through_all_x({x, y, z, w}) do
    goes_through_all_y({x - 1, y, z, w}) ++
      goes_through_all_y({x, y, z, w}) ++
      goes_through_all_y({x + 1, y, z, w})
  end

  def goes_through_all_y({x, y, z, w}) do
    goes_through_all_z({x, y - 1, z, w}) ++
      goes_through_all_z({x, y, z, w}) ++
      goes_through_all_z({x, y + 1, z, w})
  end

  def goes_through_all_z({x, y, z, w}) do
    goes_through_all_w({x, y, z + 1, w}) ++
      goes_through_all_w({x, y, z, w}) ++
      goes_through_all_w({x, y, z - 1, w})
  end

  def goes_through_all_w({x, y, z, w}) do
    [{x, y, z, w - 1}] ++
      [{x, y, z, w}] ++
      [{x, y, z, w + 1}]
  end
